...
describe('screenshot', () => {
  ...
  test('FIRST matches the reviewed baseline', async () => {
    ...
    const shot = await AndroidExp.screenshot({ includeWindow: true, targets: { first: R.id.api_ui_class_first } });
    expect(shot.ok).toBe(true);
    expect(shot.window).toMatch(/^\//);
    expect(shot.targets.first).toMatch(/^\//);
    await expect(shot.targets.first).toHaveScreenshot(BASELINE, { maxDiffPixelRatio: 0 });
  });
});

  ...
  test(`capture ${name} verifier requires runtime evidence, not RPC success`, () => {
    ...
    if (name === 'throw') {
      for (const index of [0, 1]) {
        for (const argumentTypes of [undefined, [], ['int']]) {
          const invalid = structuredClone(valid);
          invalid.chain[index].argumentTypes = argumentTypes;
          assert.throws(() => verifyEvidence(contract, invalid), { name: 'AssertionError' });
        }
      }
    }
    if (name === 'recursive') {

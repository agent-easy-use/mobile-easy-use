...
for (const platform of ['android', 'ios']) {
  const {verifyStateRuntime, verifyMethodMatching} = await import(
    `../fixtures/${platform}/ApiDemo/probe/evidence/state-runtime-contracts.mjs`);

  test(`${platform}: state runtime oracle rejects absent reads and failure disguised as null`, () => {
  ...
  test(`${platform}: matching oracle rejects alias/sibling leakage, wrong labels and missing calls`, () => {
  ...
  for (const kind of ['instance', 'class']) {
    test(`${platform}: child ${kind} matching oracle rejects parent leakage and parent labels`, () => {
